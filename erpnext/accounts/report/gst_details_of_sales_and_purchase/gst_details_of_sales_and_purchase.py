...
def get_columns():
	return [ _("ID") + ":Data:200", _("Supplier/Customer Name") + ":Data:160", _("Date") + ":Date:120",  
		_("Rate") + ":Float:120", _("Sales Value") + ":Float:120", _("Purchase Value") + ":Float:120", 
		_("GST Collected") + ":Float:150",_("GST Paid") + ":Float:150"]
...
def get_result(filters):
	...
	for d in frappe.get_all("GST Types",fields=["name","gst_type_abbreviation"],order_by="name"):
		...
		row = frappe.db.sql("""select COALESCE(form_id,CONCAT('<b>','Total','</b>')),(case when form_id is null then null else 
			...
			null then '' else rate end),0) as rate, sum(ifnull(output_sales_value,0)), sum(ifnull(input_purchase_value,0)), 
			sum(ifnull(output_gst_collected,0)), sum(ifnull(input_gst_paid,0)) from `tabGST Details` where cs_gst_type = %s %s 
	...
	total_value = frappe.db.sql("""select sum(ifnull(output_gst_collected,0)) ,sum(ifnull(input_gst_paid,0)) from 
